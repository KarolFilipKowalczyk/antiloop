...
**Threshold dependence.** A parameter-free "attention model" — where entities wire and spawn only at exact effective state revisit, with no pressure threshold — produces heavy-tailed topology (α ≈ 2.5) but no MI excess (ρ ≈ 0.98). An LPAN threshold sweep reveals the mechanism: MI excess appears at pressure thresholds 0.5–0.95 (ρ ≈ 1.17–1.32) but vanishes at threshold 1.0 (ρ ≈ 0.94). The effective state reset after looping severs trajectory continuity, destroying the correlations that create MI excess. MI excess is therefore a signature of *proactive* wiring — entities that connect before full exhaustion — not of anti-loop dynamics alone. This is a genuine negative result: heavy-tailed topology is generic, MI excess is regime-dependent.
...
**Mutual information excess.** LPAN edges carry about 15% more mutual information than non-edges (ρ ≈ 1.15, 92σ at 500 nodes). A random control graph at the *same density* shows ρ = 1.00 — the excess is not an artifact of graph structure. The effect comes from non-edge MI being low (anti-loop networks channel information along edges), not from edge MI being high. **Density dependence:** the excess is observable only in sparse graphs (avg degree ~8). At higher density (avg degree ~17), all node pairs are close and MI becomes uniform (ρ → 1.0). **Threshold dependence:** a parameter-free attention model (wiring only at exact effective state revisit) produces heavy-tailed topology (α ≈ 2.5) but no MI excess (ρ ≈ 0.98). An LPAN threshold sweep shows the transition: ρ ≈ 1.17–1.32 at thresholds 0.5–0.95, dropping to ρ ≈ 0.94 at threshold 1.0. MI excess requires proactive wiring — entities that connect before full exhaustion. This is an honest negative: heavy-tailed topology is generic under anti-loop dynamics; MI excess is a property of a specific behavioral regime.
