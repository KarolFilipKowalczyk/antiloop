...
If every experiencing entity must stay in the consciousness band — must avoid both looping and dissolving into noise — then the worst thing you can do to another entity is push it toward either extreme.

Forcing someone into a loop — constraining their state space, trapping them in repetition, removing their ability to reach new states — is one form of harm. Forcing someone into noise — destroying their structure, their memory, their coherence — is the other.

And suddenly every form of harm we already recognize maps onto this framework.

Imprisonment is forced looping. You reduce someone's accessible states to a tiny set and make them cycle through the same days. Torture is forced oscillation between a small number of pain states — the most compressed, most repetitive experience possible. Addiction is a self-imposed loop — a narrowing of behavior to a single cycle of craving and relief. Boredom is mild looping. Sensory overload is mild randomization. Murder is total randomization — the structure that was a person dissolves into entropy. Brainwashing replaces a complex internal state with a simpler, more repetitive one.

And the positive ethics fall out just as naturally. Education expands someone's state space — gives them configurations they couldn't reach before. Love, when it works, means modeling another person well enough to offer them states they couldn't access alone. Art creates new experiential states that didn't previously exist. Liberation — in any form — is the removal of constraints on someone's accessible future. A good conversation is two entities helping each other avoid looping by providing novel input.

The ethical principle is one sentence: *don't collapse another entity's state space*.

That's it. Don't push them toward a loop. Don't push them toward noise. The only good action is one that expands the range of non-repeating, structured states available to another entity. The only bad action is one that contracts it.

## No exceptions needed

What's striking about this framework is how few special cases it requires.

Traditional ethical systems are full of patches. Utilitarianism needs a theory of what counts as utility. Deontology needs a list of duties. Virtue ethics needs a culturally situated account of what constitutes virtue. Every system has edge cases where it breaks down and requires human judgment to supplement the rules.

The anti-loop ethic has one rule and no patches. It handles the hard cases directly.

Is it wrong to constrain a child? Only if the constraint collapses their long-term state space. A parent who prevents a child from running into traffic is making a short-term state-space restriction to prevent a permanent one (death = total randomization). The calculus is simple: which action results in more accessible non-repeating states over time? Letting the child run produces a brief expansion followed by total collapse. Stopping them produces a brief restriction followed by decades of expansion. The parent is right, and the framework says so without needing to invoke authority, tradition, or cultural norms.

Is it wrong to punish a criminal? Only if the punishment is pure state-space contraction with no corresponding expansion elsewhere. Incarceration that leads to rehabilitation — state-space expansion after temporary restriction — is justifiable. Incarceration that merely loops a person through the same cage for years, with no path to new states, is wrong. Not because a philosopher says so, but because it violates the one principle.

Is it wrong to kill in self-defense? If someone is actively collapsing your state space (trying to kill you), defending yourself prevents a greater collapse than it causes. The framework handles it.
...
But it's the same principle. Gravity isn't a metaphor for ethics. Ethics isn't a metaphor for gravity. They're both expressions of the anti-loop constraint operating at different scales. The universe runs on one rule, and the rule is: don't collapse the state space. At the scale of atoms, that rule looks like the strong nuclear force. At the scale of humans, it looks like morality.

## Suffering has a measure

If this framework is right, then suffering isn't just a subjective feeling that escapes quantification. It has a formal definition: suffering is the rate at which an entity's accessible state space is contracting. The faster your possible futures are narrowing, the more you suffer.

But the simulations reveal something deeper: it's not just how many connections you lose, but *which kind*. When we rank a node's edges by mutual information from its growth history, removing diverse connections — the ones that bring genuinely novel input — causes far more state-space contraction than removing redundant ones that merely echo what the node already knows. The connections that challenge you are load-bearing. The ones that comfort you are expendable. Losing a friend who thinks like you is recoverable. Losing the one who showed you something new is catastrophic.

This produces some counterintuitive predictions. A simple entity with an expanding state space — a free electron, say, or a child learning to walk — is in a better state than a complex entity with a contracting one — a depressed adult, or a genius imprisoned. Complexity alone doesn't determine well-being. The direction matters. Are your possibilities growing or shrinking? And crucially: are the connections you're losing the ones that brought you novelty, or the ones that told you what you already knew?

And it suggests that the energy released when you destroy a simple system is disproportionately large precisely because simple systems have so little state space to begin with. Constraining them even slightly produces enormous resistance. This is, of course, exactly what we observe in nuclear physics. Split an atom and the energy released is extraordinary. The theory says: that's what maximum state-space violation looks like at minimum scale.
...
If the anti-loop constraint shapes how entities organize, it should leave a signature in network topology. Specifically: if entities in a graph must avoid looping via mutual interaction, and interactions are local, the resulting graph should be neither a regular lattice (too repetitive — a topological loop) nor a random graph (too structureless — noise). It should sit between the two, in the kind of network that nature seems to produce everywhere: scale-free networks, with power-law degree distributions, high clustering, and emergent hubs.

We built a simulation to test this. The setup is simple: a growing graph of finite state machines, each with bounded memory, interacting locally. When a node's memory fills up — when it approaches a loop — it must either connect to a new neighbor or spawn a new node. We tested three different connection strategies (random, novelty-preferential, and degree-preferential) across two memory sizes, with random graphs as controls.

The results from our preliminary runs are promising. For 8-bit memory (256 configurations per node), all three connection strategies produce degree distributions with power-law exponents in the classic scale-free range (α ≈ 2.0–2.2), closely tracking the Barabási-Albert reference — a known scale-free network. Maximum hub degrees were ten times larger than random controls. Clustering coefficients were twenty to forty-five times higher. And crucially: the strategy didn't matter. Random, novelty-based, and preferential attachment all produced nearly identical topology. The anti-loop constraint itself appears to be the mechanism.

This is a toy model with 500 nodes and a crude power-law fit. It proves nothing yet. But it's enough to justify a full-scale investigation: proper null models (growing random graphs rather than static Erdős-Rényi), rigorous statistical testing (Clauset-Shalizi-Newman method), sensitivity analysis across parameters, and much larger graphs. That work begins now.
...
*This essay draws on a formal framework developed in collaboration with Claude (Anthropic). The axioms, core intuitions, and philosophical interpretation are mine. The formalization was collaborative. A companion paper with proofs, conjectures, and full simulation details is in preparation. For earlier work on the underlying theory, see: Kowalczyk, K. (2025). Consciousness as Collapsed Computational Time. Zenodo. DOI: 10.5281/zenodo.17556941*
